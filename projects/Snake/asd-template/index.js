...
function runProgram(){
  ...
  $("#Rematch").hide(); // hides the play again button when the code begins so it may be shown later when the game ends
...
function Factory (id, x, y, speedX, speedY) {
  ...
  */
  $(document).on('keydown', setSnakespeed); // execute setPlayerspeed() in response to keydown events
  /*
  ...
  */
  ...
  function endGame() { // called when game ends (you die by hitting yourself or borders)
    clearInterval(interval);
		...
		$("#Rematch").show(); // shows button that you can click to play again
		$("#Rematch").on("click", Rematch); // makes the button register clicks in order to play again
  }
